...
await using var topic = await TemporaryTopic.CreateIfNotExistsAsync(
  "<fully-qualified-namespace>", "<topic-name>", logger, CancellationToken.None);
```
...
await topic.AddSubscriptionAsync("<subscription-name>", CancellationToken.None);
```
...
await using var sub = await TemporaryTopicSubscription.CreateIfNotExistsAsync(
    "<fully-qualified-namespace>", "<topic-name>", "<subscription-name>", logger, CancellationToken.None);
```
...
await sub.AddRuleIfNotExistsAsync("<rule-name>", filter, CancellationToken.None);
```
...
await using var queue = await TemporaryQueue.CreateIfNotExistsAsync(
  "<fully-qualified-namespace>", "<queue-name>", logger, CancellationToken.None);
```
